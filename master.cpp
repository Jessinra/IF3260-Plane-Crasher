...
void Master::draw(int xStart, int yStart, int **img, int height, int width) {
    for (int y = max(0, -yStart); y < height; y++) {
        if (y + yStart >= this->yend) break;
        for (int x = max(0, -xStart); x < width; x++) {
            if (x + xStart >= this->xend) break;
...
void Master::drawPixels(int xStart, int yStart, Pixel *pixels) {
    for (Pixel &p : pixels) {
...
void Master::drawLine(int positionX, int positionY, const Line &line) {
    ...
    if (xStart == xEnd) {
        if (xStart + positionX >= this->xstart && xStart + positionX < this->xend) {
...
void Master::drawObject(const Object &obj) {
    int objectX = obj.getRefPos().getX();
    int objectY = obj.getRefPos().getY();
    for (const Line &ln : obj.getRefLines()) {
        drawLine(objectX, objectY, ln);
    }
}